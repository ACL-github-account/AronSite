...
window.onload = function(){
	var incompat = 0;
	try {
		window.localStorage.highcont;
		window.localStorage.csans;
	}
	catch(eer) {
		incompat = 1;
	}
	switch (incompat){
		case 0:
			...
			if (window.localStorage.highcont == 1){
			...
			if (window.localStorage.csans == 1){
				let tar = document.getElementsByTagName("*");
				for (let x = 0; x<tar.length; x++) {
			...
			function highcontrastoption(e){
				if(window.localStorage.highcont == 1){
					window.localStorage.highcont = 0;
				}
				else {
					window.localStorage.highcont = 1;
				}
				location.reload();
			}
			function csansoption(e){
				if(window.localStorage.csans == 1){
					window.localStorage.csans = 0;
				}
				else {
					window.localStorage.csans = 1;
				}
				location.reload();
			}
